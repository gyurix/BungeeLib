...
public class Command implements StringSerializable {
  ...
  static {
    ...
    registerExecutor((sender, text) -> {
      BU.pm.dispatchCommand(BU.cs, text);
      return true;
    }, "CONSOLE");
    registerExecutor((sender, text) -> {
  ...
  public Command(String in) {
    ...
    if (d[0].startsWith("{")) {
      int len = d[0].indexOf("}");
      delay = Long.parseLong(d[0].substring(1, len));
  ...
  public static void executeAll(Iterable<Command> cmds, CommandSender sender) {
    if (cmds == null)
      return;
    executeAll(cmds.iterator(), sender);
  }

  public static void executeAll(Iterator<Command> cmds, CommandSender sender) {
    if (cmds == null)
      return;
    while (cmds.hasNext()) {
